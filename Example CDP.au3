...
Func ElementAttributes()
	...
	$chrome = LaunchStandardChrome()
	$page    = $chrome.page()

	$page.goto("https://testpages.eviltester.com")

	$page.locator("//a[@href='/pages/']").click(True)
	$page.locator("//a[@href='/pages/basics/']").click(True)
	$page.locator("//a[@href='/pages/basics/element-attribute-examples/']").click(True)

	$test.expect($page.locator("//p[@id='domattributes']").textContent()).toBe("This paragraph has attributes")
	$test.expect($page.locator("//p[@id='jsattributes']").textContent()).toBe("This paragraph has dynamic attributes")
	$test.expect($page.locator("//p[@id='jsattributes']").getAttribute("nextid")).toBe(1)
	$page.locator("//button[@id='add-attribute-button']").click()
	$test.expect($page.locator("//p[@id='jsattributes']").getAttribute("nextid")).toBe(2)
...
Func BasicWebPageWithStandardHeadlessBrowser()
	...
	$chrome = LaunchStandardHeadlessChrome()
	$page    = $chrome.page()
	BasicWebPage($chrome, $page)
...
Func BasicWebPageWithPlaywrightHeadlessBrowser()
	...
	$chrome = LaunchPlaywrightHeadlessChrome()
	$page    = $chrome.headlessShell()
	BasicWebPage($chrome, $page)
...
Func LaunchStandardChrome()
	Return $browser.launch(Default, 9299, Default, Default, "1280,800")
EndFunc

Func LaunchStandardHeadlessChrome()
	Return $browser.launch(Default, 9299, "--headless=new")
EndFunc

Func LaunchPlaywrightChrome()
	Return $browser.launch(@LocalAppDataDir & '\ms-playwright\chromium-1217\chrome-win64\chrome.exe', 9299, Default, Default, "1280,800")
EndFunc

Func LaunchPlaywrightHeadlessChrome()
	Return $browser.launch(@LocalAppDataDir & '\ms-playwright\chromium_headless_shell-1217\chrome-headless-shell-win64\chrome-headless-shell.exe', 9299)
EndFunc
