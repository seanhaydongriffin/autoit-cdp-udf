...
struct json_object **json_object_array_get_objects(struct json_object *json_array)
{
	if (!json_array || !json_object_is_type(json_array, json_type_array))
	{
		return NULL; // Ensure we are working with a valid JSON array
	}

	int length = json_object_array_length(json_array);

	if (length <= 0)
		return NULL;

	struct json_object **result = malloc(length * sizeof(struct json_object *));
	...
	{
		result[i] = json_object_array_get_idx(json_array, i);
	}

	return result;
}

struct json_object **json_object_array_get_objects_w_arraylist(struct json_object *json_array)
{
	if (!json_array || !json_object_is_type(json_array, json_type_array))
	{
		return NULL; // Ensure we are working with a valid JSON array
	...
	struct array_list *arraylist = json_object_get_array(json_array);

	int length = array_list_length(arraylist);
	...
	for (int i = 0; i < length; i++)
	{
		result[i] = array_list_get_idx(arraylist, i);
	}
...
void *json_object_get_value(struct json_object *obj, int *outType, int *outLength)
{
	if (!obj || !outType)
		return NULL;

	*outType = json_object_get_type(obj); // Set the type
	...
	switch (*outType)
	{
		...
		case json_type_string: 
		{
		    char *tmp = json_object_get_string(obj);
		    *outLength = strlen(tmp);
		    return (void *)tmp;
	    }

		case json_type_object:
	    {
		    void *tmp = json_object_get_object(obj);
		    *outLength = json_object_object_length(tmp);
		    return (void *)tmp;
	    }

		case json_type_array:
	    {
		    void *tmp = json_object_get_array(obj);
		    *outLength = json_object_array_length(tmp);
		    return (void *)tmp;
	    }
...
void *json_object_get_field_value(const struct json_object *jso, const char *key, int *outType, int *outLength)
{
	if (!jso || !key || !outType) return NULL;
	struct json_object *fred = json_object_object_get(jso, key);
	if (!fred) return NULL;
	//return json_object_get_value(fred, outType);


	//void *result = json_object_get_value(fred, outType, outLength);
	//return result;

	return json_object_get_value(fred, outType, outLength);
